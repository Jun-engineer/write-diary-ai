...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    if (plan === 'premium') {
      return forbidden('Premium users have unlimited corrections');
    }
    ...
    if (currentBonusCount >= maxBonus) {
      return forbidden(JSON.stringify({
        ...
        message: 'Maximum bonus corrections already granted for today',
        bonusCount: currentBonusCount,
        maxBonus,
      }));
    }

    // Grant bonus correction
    ...
    return success({
      success: true,
      message: 'Bonus correction granted',
      bonusCount: newBonusCount,
      maxBonus,
      remainingBonus: maxBonus - newBonusCount,
    });
  } catch (error) {
    console.error('Grant bonus correction error:', error);
    return serverError('Failed to grant bonus correction');
  }
};
